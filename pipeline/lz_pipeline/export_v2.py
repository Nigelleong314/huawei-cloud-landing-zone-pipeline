...
EXCLUDE_NAMES = {"secrets.auto.tfvars.json", "errored.tfstate", "tf.plan",
                 "plan.json", ".lzctl.lock", ".gitignore"}
# plan files embed variable values (incl. secrets) - never ship any of them,
# in ANY rendering: the binary tf.plan/.tfplan AND `terraform show -json`
# output (plan.json / *.plan.json). The JSON form carried the master AK/SK
# into a real handover export before this rule existed (frasers, 2026-09-08).
# .txt: stray plan/log captures
EXCLUDE_SUFFIXES = (".bak", ".backup", ".ps1", ".py", ".xlsx", ".txt",
                    ".tfplan", ".plan.json", ".log")
