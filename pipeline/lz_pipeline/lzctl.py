...
def cmd_assess(args):
    ...
    lines = [f"# Decisions needed - {slug}", "",
             ...
             "Every field lands in exactly one bucket; nothing below was guessed.", "",
             f"## Open questions ({len(gaps)}) - resolve before build", ""]
    lines += [f"- **{r}** {q}  \n  targets: `{t}`" for r, q, t in gaps] or ["(none)"]
    lines += ["", f"## Defaults applied if left silent ({len(defaulted)}) - review", ""]
