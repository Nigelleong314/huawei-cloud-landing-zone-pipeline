...
render() returns the rendered text; render_lines() the line list, matching the
emitters' compose-a-list-of-lines idiom so converted and unconverted emitters
mix freely while every byte stays golden-tested.
"""
...
def render(name: str, **vars) -> str:
    ...
    return string.Template(text).substitute(**vars)
...
def render_lines(name: str, **vars) -> list:
    ...
    return render(name, **vars).splitlines()
