...
resource "huaweicloud_cfw_acl_rule" "this" {
  ...
  dynamic "custom_services" {
    ...
    content {
      # icmp: the API does not store ports, so the plan re-shows them
      # on every run (cosmetic).
      protocol    = local.proto_num[split("/", custom_services.value)[0]]
      source_port = split("/", custom_services.value)[1] == "any" ? "1-65535" : split("/", custom_services.value)[1]
      dest_port   = split("/", custom_services.value)[2] == "any" ? "1-65535" : split("/", custom_services.value)[2]
    }
  }
