...
Usage: python fill_questionnaire.py BLANK.xlsx OUT.xlsx SECRET

Leaves C9, D5, D19 blank (no default -> OPEN items) and pastes SECRET into
the prose of C13 to exercise pre-model redaction end to end.
"""
import sys
from pathlib import Path
import openpyxl

blank, out, secret = sys.argv[1], sys.argv[2], sys.argv[3]
...
ANSWERS = {
 ...
 "C13": f"Site-to-site VPN from DC1 to Huawei Cloud to start; our firewall team noted the pre-shared key is {secret} for the pilot tunnel. Direct Connect maybe next year.",
...
wb.save(out)
print(f"filled {filled} answers + appendix rows -> {Path(out).name}")
