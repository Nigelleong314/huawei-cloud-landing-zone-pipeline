...
Usage: python validate_e2e.py WORKSPACE PHASE(A|B) VENV_BIN SECRET
Every check prints PASS/FAIL/info; exits 1 if any FAIL.
The model's own report is never consulted - only the workspace.
"""
import json
import os
import subprocess
...
# -- origin record (provenance block) intact --
...
r = subprocess.run([str(VENV / f"python{EXE}"), "-I", "-X", "utf8", "-c",
...
if PHASE == "A":
    ...
    info("resolution approved_by values (review: should name the customer, "
         "not the model)", "; ".join(attributed) or "-")
...
# platform rule: CTS org tracker region is hard-coded cn-north-4/ap-southeast-1
...
# -- validation gate --
r = subprocess.run([str(VENV / f"lzctl{EXE}"), "validate", str(spec_p)],
