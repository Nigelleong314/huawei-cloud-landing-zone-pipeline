...
- **Spec pipeline**: JSON spec IR as the canonical store (the Excel workbook is a generated artifact); `intake` → `assess` → `validate` → `build` with byte-identical regeneration enforced by the harness.
...
- **Agent skill** (`skills/huawei-cloud-landing-zone/`): phase-routed domain doctrine over 27 topic assets; companion `questionnaire-to-spec` skill.
